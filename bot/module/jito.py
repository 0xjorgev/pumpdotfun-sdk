...
async def send_transaction_with_priority_fee(
    sdk,
    solana_client,
    sender,
    receiver,
...
async def confirm_landed_bundle(sdk: JitoJsonRpcSDK, bundle_id: str, max_attempts: int = 60, delay: float = 2.0):
    for attempt in range(max_attempts):
        ...
        if 'result' not in response['data']:
            print("Unexpected response structure. 'result' not found in response data.")
            await asyncio.sleep(delay)
...
async def basic_bundle():
    ...
    solana_client = AsyncClient(appconfig.JITO_RPC_URL)

    wallet_keypair = Keypair.from_base58_string(appconfig.PRIVKEY)
    # Initialize JitoJsonRpcSDK
    jito_client = JitoJsonRpcSDK(url="{}/api/v1".format(appconfig.JITO_RPC_URL))
...
async def basic_txn():
    ...
    sdk = JitoJsonRpcSDK(url=appconfig.JITO_RPC_URL)
